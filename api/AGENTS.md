# Orientation for AI agents working in `api/`

> **Start here if you're an AI (Claude / Cursor / etc.) editing this folder.**
> Full context lives in [`../doc/api-integration.md`](../doc/api-integration.md). This file is the 60-second version.
...
## What this is NOT
- Not the web app. Web app is in `../src/`.
- Not Supabase Edge Functions. Those live in `../supabase/functions/` and use JWT auth. Do not duplicate logic here — but DO port logic cleanly if agents need it (the AI service + Threads publisher are examples of correct ports).
...
| I want to... | Do this |
|---|---|
| Add a new endpoint | Edit the relevant `src/routes/<domain>.js`, declare scope with `requireScope(...)`, keep handler thin. Update `mcp/server.js` TOOLS array. Update `README.md` + `../doc/api-integration.md` §9. |
| Wire a new social platform | See `../doc/api-integration.md` §7. Add service file, update `services/publishers.js` dispatcher. |
| Add a new AI provider | See `../doc/api-integration.md` §8. Extend `services/ai.js`. |
